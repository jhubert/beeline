...
// Read-only access + draft creation. `gmail.drafts.create` is the narrowest
// scope that allows creating drafts: it is create-only and cannot send (unlike
// `gmail.compose`). We never expose a send tool — drafts are reviewed and sent
// by the user in their mail app.
